...
def playlist_save(playlist):
    ...
    pl = (
        ...
        if ((not "id" in playlist) or (playlist["id"] == 0))
        else Playlists.get_playlist(playlist["id"])
    )
    pl.clear_elements()
...
def settings_request():
    ...
    settings["buttons"]["available"] = app.bmanager.gpio_is_available() or int(
        os.getenv("DEV_HWBUTTONS", default="0")
    )
    settings["leds"]["available"]["value"] = app.lmanager.is_available() or int(
    ...
    ] = app.umanager.docker_compose_latest_version
...
def queue_set_order(elements):
    if elements == "":
        ...
        app.qmanager.set_new_order(
            map(lambda e: ElementsFactory.create_element_from_dict(e), json.loads(elements))
        )
