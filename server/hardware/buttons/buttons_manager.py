...
import inspect
from server.hardware.buttons.generic_button_event import (
    GenericButtonAction,
    GenericButtonEventManager,
)
import server.hardware.buttons.actions as button_actions
    ...
    def __init__(self, app):
        ...
        try:
            ...
            self.app.logger.error(
                "buttons:The GPIO is not accessible. If you are using a raspberry pi be sure to use superuser privileges to run this software. \n    Be sure to check also the installation instructions dedicated to the hw options\n    If the error persist open an issue on github"
            )
        ...
        for cl in inspect.getmembers(button_actions, inspect.isclass):
            if not cl[1] == GenericButtonAction:
                ...
                self.available_buttons_actions.append(
                    {
                ...
                self._actions[cl[0]] = cl[
                    1
                ]  # storing classes in an array to instantiate the actions later
    ...
    def update_settings(self, settings):
        if self.gpio_is_available():
            ...
            if not self._buttons is None:
                ...
                ):  # should update only if any difference has been found or if the self._button object is None
                    should_update = False
            ...
            for b in self._buttons:
                ...
                bobj.set_long_press_action(
                    self._actions[self._labels[b["press"]["value"]]](self.app)
                )
                ...
                GPIO.add_event_detect(
                    pin, GPIO.BOTH, callback=bobj.button_change, bouncetime=BOUNCE_TIME
                )  # the rising or falling edge is detected in the button event class
