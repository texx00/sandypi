...
from server.hardware.leds.leds_types.dimmable import Dimmable
from server.hardware.leds.leds_types.RGB_neopixels import RGBNeopixels
from server.hardware.leds.leds_types.RGBW_neopixels import RGBWNeopixels
from server.hardware.leds.leds_types.WWA_neopixels import WWANeopixels
from server.hardware.leds.light_sensors.tsl2591 import TSL2591
...
class LedsController:
    ...
    def update_settings(self, settings):
        ...
        dims = (
            int(settings.leds.width),
            int(settings.leds.height),
            int(settings.leds.circumference),
        )
        ...
        if (self.leds_type != settings.leds.type) or (self.pin != settings.leds.pin1):
            ...
            try:
                ...
                leds_number = (
                    (int(self.dimensions[0]) + int(self.dimensions[1])) * 2
                    if settings.device.type == "Cartesian"
                    else int(self.dimensions[2])
                )
                leds_class = Dimmable
            ...
            try:
                ...
                if self.is_available():
                    self.app.semits.show_toast_on_UI(
                        "The select sensor is not compatible with the current setup"
                    )
