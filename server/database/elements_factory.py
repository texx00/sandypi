...
class ElementsFactory:
    ...
    def create_element_from_dict(cls, dict_val):
        ...
        if "element_type" in dict_val:
            # removing the element type. Should be already be choosen when using the class
            el_type = dict_val.pop("element_type")
        else:
            raise ValueError("the dictionary must contain an 'element_type'")

        # need to import here to avoid circular import
        from server.database.playlist_elements import (
            _get_elements_types,
        )
