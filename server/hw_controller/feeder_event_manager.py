...
class FeederEventManager(FeederEventHandler):
    ...
    def on_new_line(self, line):
        ...
        # Avoid sendind every message, send them every maximum 0.5s (if they are fast, a 0.5s difference should not change much in the drawing preview)
        if time.time() - self.last_send_time > 0.5 or self.command_index < 16:
