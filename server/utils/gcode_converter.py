...
class ImageFactory:
    ...
    def __init__(
        ...
        # color argument requires also alpha value
        self.bg_color = bg_color if len(bg_color) == 4 else (*bg_color, 0)
    ...
    def update_device(self, device):
        ...
        if self.is_scara():
            ...
            # for scara robots follow https://forum.v1engineering.com/t/sandtrails-a-polar-sand-table/16844/61
            self.pi_conversion = pi / float(device["angle_conversion_factor"])
            self.device_radius = float(device["radius"])
            # *2 for the conversion factor (will spare one operation in the loop)
            self.offset_1 = float(device["offset_angle_1"]) * 2
            # *2 for the conversion factor (will spare one operation in the loop)
            self.offset_2 = float(device["offset_angle_2"]) * 2
        elif self.is_polar():
    ...
    def gcode_to_coords(self, file):
        ...
        for line in file:
            ...
            # parsing line
            params = line.split(" ")
            if not (params[0] in self.straight_lines):
                # TODO include also G2 and other curves command?
                if self.verbose:
    ...
    def draw_image(self, coords, drawing_infos):
        ...
        p_1 = coords[0]
        self.circle(
            d, (remapx(p_1[0]), remapy(p_1[1])), self.line_width * factor / 2, self.line_color
        )  # draw a circle to make round corners
        for p in coords[1:]:  # create the line between two consecutive coordinates
            d.line(
                [remapx(p_1[0]), remapy(p_1[1]), remapx(p[0]), remapy(p[1])],
                fill=self.line_color,
                width=int(self.line_width * factor),
            )
            if self.verbose:
